...
object PerioderMedInaktivitetMapper {

    /* @JvmStatic
     fun tilPerioderMedInaktivitet(cv: Cv): EsPerioderMedInaktivitet {
         val aktivePerioder = perioderMedAktivitet(cv)
         ...
         val inaktivePerioderEtterEnVissAlder =
             inaktivePerioderEtterEnVissAlder(inaktivePerioderMedVarighetToÅrEllerMer, cv.foedselsdato)
     ...
     private data class AktivPeriode(val førsteDag: LocalDate, val sisteDag: LocalDate) {
     ...
     private fun perioderMedAktivitet(cv: Cv): List<AktivPeriode> {
         val arbeidserfaring = cv.arbeidserfaring.map {
             AktivPeriode(
                 it.fraTidspunkt ?: LocalDate.MIN,
                 safeToLocalDate(it.tilTidspunkt) ?: LocalDate.MAX
             )
         }

         val utdanning = cv.utdannelse.map {
             AktivPeriode(
                 it.fraTidspunkt ?: LocalDate.MIN,
                 safeToLocalDate(it.tilTidspunkt) ?: LocalDate.MAX
             )
         ...
         return arbeidserfaring + utdanning
     }
     ...
     private fun flattened(potensieltOverlappendePerioder: Collection<AktivPeriode>): List<AktivPeriode> {
         ...
         potensieltOverlappendePerioder.forEach { add(it) }
     ...
     private fun esPerioderMedInaktivitet(
         ...
         val esFørsteDagIInneværendeInaktivePeriode =
             if (førsteDagIInneværendeInaktivePeriode == LocalDate.MIN || førsteDagIInneværendeInaktivePeriode == LocalDate.MAX) null
             else safeToDate(førsteDagIInneværendeInaktivePeriode)

         val sluttdatoerForInaktivePerioder = inaktivePerioder.map { safeToDate(it.sisteDag) }

         return EsPerioderMedInaktivitet(esFørsteDagIInneværendeInaktivePeriode, sluttdatoerForInaktivePerioder)
     }
