...
class HullCvLytterTest {
    ...
    /*@Test
    fun `Lag inaktive perioder av cv-melding`() {
        val testRapid = TestRapid()
        startApp(testRapid)

        testRapid.sendTestMessage(eksempelHendelse)

        val inspektør = testRapid.inspektør
        Assertions.assertThat(inspektør.size).isEqualTo(1)

        val melding = inspektør.message(0)
        Assertions.assertThat(melding["hullICv"].isArray).isTrue
        Assertions.assertThat(melding["hullICv"][0]["førsteDagIInneværendeInaktivePeriode"]).isEqualTo(PerioderMedInaktivitet(
            LocalDate.now(),))
        Assertions.assertThat(melding["hullICv"][0]["sluttdatoerForInaktivePerioder"]).isEqualTo()
    }

    val eksempelHendelse = """{
  "aktørId": "1000100281879",
  "cv": {
    "opprettCv": null,
    "endreCv": {
      "cv": {
  ...
  "system_participating_services": [
    {
      "service": "toi-tilretteleggingsbehov",
      "instance": "toi-tilretteleggingsbehov-6dbf4df4d-krnll",
      "time": "2022-02-24T12:00:34.454621266"
    },
    {
      "service": "toi-sammenstille-kandidat",
      "instance": "toi-sammenstille-kandidat-dc5956cc5-k7pmm",
      "time": "2022-02-24T12:14:24.128144473"
    },
    {
      "service": "toi-synlighetsmotor",
      "instance": "toi-synlighetsmotor-5979b4f7fc-t78mw",
      "time": "2022-02-24T12:26:27.396410443"
    }
  ],
  "@event_name": "tilretteleggingsbehov.sammenstilt",
}"""*/
