...
data class DirektemeldtStilling(
    ...
    fun konverterTilStilling(stillingskategori: Stillingskategori?, stillingsInfo: Stillingsinfo): Stilling = Stilling(
        ...
        employer = innhold.employer?.let { it.copy(properties = it.properties.filterValues { value -> value != null }) },
        locationList = innhold.locationList,
        categoryList = innhold.categoryList,
        properties = innhold.properties.entries.mapNotNull { (key, value) -> value?.let { key to it } }.toMap() + ("direktemeldtStillingskategori" to (stillingskategori?.name ?: "")),
